...
export default function AppDialog({ title, isOpen, onClose, preventClose, children }: Props) {
	...
	return createPortal(
		<div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
			<div className="absolute inset-0 bg-black/70 backdrop-blur-sm" onClick={() => !preventClose && onClose()} />
			<section
				role="dialog"
				aria-modal="true"
				className="relative bg-card-body-primary border border-card-content-highlight p-4 flex flex-col gap-y-4 w-full max-w-lg max-h-[90vh] overflow-y-auto"
			...
			>
				...
				<div className="flex items-center">
					...
					<button
						className={`text-text-secondary ${preventClose ? "cursor-not-allowed" : "hover:text-card-content-highlight"}`}
