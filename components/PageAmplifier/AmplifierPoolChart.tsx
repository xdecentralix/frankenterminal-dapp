...
const OTHER_COLOR = "#3A3A3A"; // de-emphasized context: everyone else's positions
const PRICE_COLOR = "#FFB000";
const ANCHOR_COLOR = "#888888";
const GRID_COLOR = "#2A2A2A";
const LABEL_COLOR = "#888888";
const MONO_FONT = "var(--font-ft-mono), monospace";
...
export default function AmplifierPoolChart({ stats, priceView, positions, overwrite }: Props) {
	...
	const ownColor = themeAccent; // highlight: positions of the acting account
	...
	const seriesColors = blocks.map((block) => (block.isOwn ? ownColor : OTHER_COLOR)).reverse();
			...
			<div className="flex flex-row justify-center gap-6 text-sm text-text-secondary">
				{hasOwn && (
					<div className="flex items-center gap-2">
						<span className="h-3 w-3" style={{ backgroundColor: ownColor }}></span>
						Your positions
					</div>
				)}
				{hasOther && (
					<div className="flex items-center gap-2">
						<span className="h-3 w-3 border border-card-input-border" style={{ backgroundColor: OTHER_COLOR }}></span>
			...
			<div className="-mx-2">
				<ApexChart
					...
					options={{
						...
						colors: seriesColors,
						stroke: { curve: "stepline", width: 1.5, colors: [GRID_COLOR] },
						...
						xaxis: {
							...
							labels: {
								...
								style: { colors: LABEL_COLOR, fontSize: "11px", fontFamily: MONO_FONT },
							},
							axisBorder: { show: true, color: GRID_COLOR },
							axisTicks: { show: true, color: GRID_COLOR },
							title: { text: priceUnit, style: { color: LABEL_COLOR, fontSize: "12px", fontWeight: 400 } },
						...
						yaxis: {
							...
							title: {
								...
								style: { color: LABEL_COLOR, fontSize: "12px", fontWeight: 400 },
							},
						},
						annotations: {
							xaxis: [
								{
									...
									label: {
										...
										style: { color: "#0A0A0A", background: PRICE_COLOR, fontSize: "11px" },
									},
								},
								...(priceView.anchor > 0
									? [
											{
												...
												label: {
													...
													orientation: "horizontal",
													offsetY: 20,
													style: { color: "#0A0A0A", background: ANCHOR_COLOR, fontSize: "11px" },
												},
											},
									  ]
									: []),
						...
						tooltip: {
							...
							custom: ({ dataPointIndex }: { dataPointIndex: number }) => {
								...
								const rows = blocks
									.map((block) => {
										...
										return `<div style="display:flex;align-items:center;gap:6px;margin-top:2px;">
											<span style="width:8px;height:8px;background:${block.isOwn ? ownColor : OTHER_COLOR};"></span>
								...
								return `<div style="padding:8px 12px;font-size:12px;min-width:200px;">
									...
									${rows || `<div style="color:${LABEL_COLOR};">No liquidity at this price</div>`}
								</div>`;
							},
						},
					}}
